...
- **Umbrella fan-out search**: `zvec_search` at a root with no index (and none on any ancestor) automatically runs the query inside every depth-1 nested repo that HAS an index and merges the results under per-repo headers (≤40 repos, 5 concurrent, ≤5 hits each). A session at a slim umbrella root — a superproject whose code lives in submodules — can therefore search every repo under it with one call. `zvec_index`'s umbrella refusal and the auto-index hook conspire to make this work: the umbrella root is never indexed (an ancestor index would lock out the repos below), and when a session starts at one, the hook indexes its depth-1 submodule repos in the background (seeded from the main checkout's bases when present).
- **Network filesystems** (`rootPolicy.allowNetworkFs`, off by default): index roots on NFS/SMB/CIFS/sshfs/… mounts are refused — building a local vector store over the network is brutally slow, and every later freshness check re-stats the tree over the wire (sessions hang the way the home-rooted index once made them). Detection is mount-table based (longest-prefix, /proc/self/mounts on Linux, `mount` elsewhere) and fail-open: if the mount table can't be read, indexing behaves exactly as before. `allowNetworkFs` or an explicit `allowRoots` entry overrides.
...
// user: ~/.pi/agent/pi-zvec-grep/config.json (values only — no scope flag)
{
	"defaultLimit": 7,
	"autoIndex": false,
	"rootPolicy": { "allowRoots": [], "maxNestedRepos": 3, "allowNetworkFs": false }
}
